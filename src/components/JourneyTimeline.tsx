    ...
    <div>
      ...
      <div className="hidden md:block">
            ...
            return (
              ...
              >
                <div className="w-full flex items-center">
                  <div className={`h-0.5 flex-1 transition-colors ${index === 0 ? "bg-transparent" : isPast || isActive ? "bg-gold" : "bg-royal/20"}`} />
                  <div
                  ...
                  >
                    <Icon className={`w-6 h-6 ${isPast ? "brand-gold-icon" : ""}`} />
                  </div>
        ...
        <div className="mt-12 bg-white rounded-3xl border border-slate-100 shadow-lg p-8 md:p-10">
          {processSteps.map((step, index) => {
            const Icon = stepIcons[index];
            return (
              <div key={step.id} className={index === active ? "block" : "hidden"}>
                <div className="flex flex-wrap items-center gap-3 mb-5">
                  ...
                  <div>
                    <p className="text-gold-dark text-xs font-bold uppercase tracking-widest">Step {step.id}</p>
                    <h3 className="font-serif text-2xl text-midnight">{step.title}</h3>
                  </div>
                </div>
                <p className="text-navy leading-relaxed max-w-3xl">{step.desc}</p>
      ...
      <div className="md:hidden space-y-4">
        {processSteps.map((step, index) => {
          ...
          return (
            <div key={step.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
              <button
                type="button"
                ...
                aria-expanded={open}
              >
                <span className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${open ? "bg-gradient-to-r from-[#C88A2B] to-[#fCDA7B] text-royal" : "bg-royal text-gold"}`}>
              ...
              {open && (
                <div className="px-6 pb-6">
                  <p className="text-navy leading-relaxed text-sm">{step.desc}</p>
                  {step.important && (
                    <div className="mt-4 flex items-start gap-3 bg-amber-50 border-l-4 border-gold rounded-xl p-4">
                      ...
                      <p className="text-xs text-navy leading-relaxed">{step.important}</p>
                    </div>
                  )}
                  {step.items.length > 0 && (
